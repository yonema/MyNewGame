...
public:
	...
	void Execute(TkmFile::SMesh& mesh, const IndexBuffer& indexBuffer, CBSP& bsp)
	{
		...
		if (texFileName.length() > 0) {
			...
			*ddsFileMemory = nsMyGame::nsMyEngine::
			...
			if (texFileFp != nullptr) {
				...
				// リソースが未登録なら、新しく生成
				if (*ddsFileMemory == nullptr)
				{
					// ddsファイルのメモリを確保
					*ddsFileMemory = new char[fileSize];
	...
	if (m_meshParts[0].isFlatShading == false) {
		...
		auto smoothFunc = [&](int startIndex, int endIndex)
		{
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				m_bpsOnVertexPosition.WalkTree(va.vertex->pos, [&](CBSP::SLeaf* leaf) {
