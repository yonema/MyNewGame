...
namespace nsMyGame
{
	...
	*/
	namespace nsAI
	{
		void CPathFinding::CellWork::Init(const CCell* cell)
		{
			this->cell = cell;
			parentCell = nullptr;
		...
		bool CPathFinding::IsIntercetRayToCell(Vector3 rayStartPos, Vector3 rayEndPos, CellWork* currentCellWork) const
		{
			// レイを計算する。
			...
			const CCell* currentCell = currentCellWork->cell;
			...
			float agentHeight)
		{
			...
			path.Clear();
			// 開始位置に最も近いセルを検索。
			const CCell& startCell = naviMesh.FindNearestCell(startPos);
			// 終了位置に最も近いセルを検索。
			const CCell& endCell = naviMesh.FindNearestCell(endPos);
			...
			while (!openList.empty()) {
				...
				for (int linkCellNo = 0; linkCellNo < 3; linkCellNo++) {
					const CCell* linkCell = crtCell->cell->GetLinkCell(linkCellNo);
					if (linkCell != nullptr) {
