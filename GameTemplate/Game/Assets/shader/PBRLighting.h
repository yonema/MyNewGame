...
float VarianceShadowMap(float3 shadowValue, float zInLVP)
{
    // 分散シャドウマップ
...
float CalcShadowRate(int ligNo, float3 worldPos)
{
    ...
    for(int cascadeIndex = 0; cascadeIndex < kMaxShadowMapNum; cascadeIndex++)
    {
        ...
        {
            // シャドウマップから値をサンプリング
            float3 shadowValue = g_shadowMap[ligNo][cascadeIndex].Sample(g_sampler, shadowMapUV).xyz;
...
float CalcPlayerShadowRate(int ligNo, float3 worldPos)
{
    ...
    {
        // シャドウマップから値をサンプリング
        float3 shadowValue = g_playerShadowMap[ligNo].Sample(g_sampler, shadowMapUV).xyz;
