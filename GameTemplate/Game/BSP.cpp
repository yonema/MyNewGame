...
#define   MAX     100
...
#endif
        Vector3 CBSP::CalcCenterPositionFromLeafList(const std::vector<SEntityPtr>& leafArray)
        {
            // まずは、AABBの中心座標を求める。
            Vector3 centerPos;
        ...
        void CBSP::CalcCovarianceMatrixFromLeafNodeList(
        ...
        void CBSP::CalcSplitPlaneFromCovarianceMatrix(
            SPlane& plane,
            float covarianceMatrix[3][3],
            const Vector3& centerPos,
        ...
        CBSP::SEntityPtr CBSP::CreateBSPTreeEntity_LeafList(const std::vector<SEntityPtr>& leafArray)
        {
        ...
        CBSP::SEntityPtr CBSP::CreateBSPTreeEntity(const std::vector<SEntityPtr>& leafArray)
        {
            if (leafArray.size() == 1) {
                // リーフエンティティを返す。
        ...
        void CBSP::WalkTree(SEntityPtr entityPtr, const Vector3& pos, std::function<void(SLeaf* leaf)> onEndWalk) const
        {
        ...
        void CBSP::WalkTree(const Vector3& pos, std::function<void(SLeaf* leaf)> onEndWalk) const
        {
